...
public class SM_ApplyBuffs : MonoBehaviour
{
  ...
  void selectTarget()
  {
    if (Input.GetKeyDown(KeyCode.R) && players.Length > 1)
    {
      in_target = Random.Range(1, players.Length);
  ...
  void increaseDamage()
  {
    ...
    {
      bl_buffActive = true;
      target.GetComponent<SM_BuffLight>().selectLight.enabled = false;
